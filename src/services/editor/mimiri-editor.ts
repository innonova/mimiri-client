...
export class MimiriEditor {
	...
	public open(note: MimerNote) {
		...
		if (note.id !== this.note?.id) {
			...
			const scrollTop = this.note.scrollTop
			this.monacoEditor.setScrollTop(scrollTop, editor.ScrollType.Immediate)
			setTimeout(() => this.monacoEditor.setScrollTop(scrollTop, editor.ScrollType.Immediate))
		} else {
			this.state.initialText = note.text
			this.state.text = note.text
		...
		this.monacoEditor.updateOptions({ readOnly: note.isCache || note.isSystem })
	}
	...
	public resetModel() {
		this.monacoEditor.setModel(this.monacoEditorModel)
		this.monacoEditor.updateOptions({ readOnly: (this.note?.isCache || this.note?.isSystem) ?? true })
	}
