<template>
	<div class="flex flex-col h-full" data-testid="change-username">
		<TabBar :items="[$t('settingsUsername.tab')]" />
		<div class="overflow-y-auto pb-10">
			<div class="grid grid-cols-[5rem_14rem] gap-3 items-baseline m-1 mt-5">
				<div class="flex items-center">{{ $t('settingsUsername.username') }}</div>
				<UsernameInput
			...
			<div class="mt-10 max-w-110 mr-2">
				...
				<div class="w-full flex justify-end mt-2 gap-2">
					<button :disabled="!canSave" @click="save" class="primary" data-testid="save-button">
						{{ $t('settingsUsername.save') }}
					</button>
				</div>
			</div>
		</div>
