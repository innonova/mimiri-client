	...
	test.only('Mobile', async () => {
		await withMimiriContext(
			async () => {
				...
				await mimiri().page.screenshot({
					path: `${process.env.SCREENSHOT_WORK_PATH}/screens/light-mobile/alice-setup.png`,
				})
				await mimiri().page.evaluate(() => (globalThis as any).mimiriApi.setDarkMode(true))
				await mimiri().page.screenshot({
					path: `${process.env.SCREENSHOT_WORK_PATH}/screens/dark-mobile/alice-setup.png`,
				})
			},
