...
test.describe('Screenshots', () => {
	test('Desktop', async () => {
		await withMimiriContext(async () => {
			...
			await mimiri().page.screenshot({ path: `${process.env.SCREENSHOT_PATH}/screens/light/alice-setup.png` })
			await mimiri().page.evaluate(() => (globalThis as any).mimiriApi.setDarkMode(true))
			await mimiri().page.screenshot({ path: `${process.env.SCREENSHOT_PATH}/screens/dark/alice-setup.png` })
		})
	})
})
